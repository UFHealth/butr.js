export let init = function (options = {}) {
  let markerOptions = options.marker || false

  const setupMarker = () => {
    if (!markerOptions || !markerOptions.use) return false
    console.log('Hello')
    let top,
      topMostSection,
      marker,
      ...
      useTo = markerOptions.useTo || false,
      callback = markerOptions.callback || null,
      duration = markerOptions.duration || 400,
      container = markerOptions.container
        ? document.querySelector(markerOptions.container)
